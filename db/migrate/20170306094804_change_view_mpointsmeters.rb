class ChangeViewMpointsmeters < ActiveRecord::Migration[5.0]
  def up
    ...
    execute("CREATE VIEW VMPOINTSMETERSMVALUES AS 
             SELECT T1.ID, T1.COMPANY_ID, T1.COD, t1.messtation, t1.meconname, T2.ID METER_ID, T2.METERNUM, T2.ACTDATE, T2.MVALUE_UPDATED_AT, T2.ACTP180, T2.ACTP280, T2.ACTP380, T2.ACTP480, T2.R      
     ...
     execute("CREATE OR REPLACE VIEW public.vmpointslnparams AS
             SELECT 
                ...
                t1.id,
                t1.name,
                t1.cod, t1.messtation, t1.meconname,
                t2.id AS ln_id,
                t2.l,
                t2.r,
                t2.ro,
