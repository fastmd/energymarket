class Remake1ViewLnparams < ActiveRecord::Migration[5.0]
  def up
    ...
    execute("CREATE OR REPLACE VIEW public.vlnparams AS
               SELECT t.id,
                  ...
                  l.wire_name,
                  l.wire_name AS mark
                 FROM (select t.*,lead(t.condate, 1, to_date('31/12/3000'::text, 'DD/MM/YYYY'::text)) OVER (PARTITION BY t.mpoint_id, t.line_id ORDER BY t.condate, t.updated_at) AS condate_end from lnparams t) t
