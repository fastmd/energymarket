class RecrViewVmpointsmetersmvalues < ActiveRecord::Migration[5.0]
  def up
    ...
    execute("CREATE OR REPLACE VIEW public.vmpointsmetersmvalues AS
                 ...
                 , CASE WHEN T.ACTP180<(LAG(T.ACTP180,1,0.0) OVER (PARTITION BY T.METER_ID ORDER BY T.ACTDATE, t.mvalue_updated_at)) THEN TRUE
                        ...
                        WHEN T.ACTP180<(LAG(T.ACTP380,1,0.0) OVER (PARTITION BY T.METER_ID ORDER BY T.ACTDATE, t.mvalue_updated_at)) THEN TRUE 
                        WHEN T.ACTP180<(LAG(T.ACTP480,1,0.0) OVER (PARTITION BY T.METER_ID ORDER BY T.ACTDATE, t.mvalue_updated_at)) THEN TRUE
