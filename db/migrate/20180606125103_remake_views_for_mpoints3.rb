class RemakeViewsForMpoints3 < ActiveRecord::Migration[5.0]
  def up
    ...
    execute("CREATE OR REPLACE VIEW public.vallmpointsproperties AS
               SELECT 
                  t.*,
                  ( SELECT v.filial_id
    ...
    execute("CREATE OR REPLACE VIEW public.vallmpoints AS
               SELECT 
                  ...
                  ( SELECT v.name
                         FROM mesubstations v
                        WHERE v.id = t.mesubstation_id) AS mesubstation_name
                 FROM mpoints t  ORDER BY  t.company_id, t.id;")                
