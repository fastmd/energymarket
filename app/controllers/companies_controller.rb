          ...
          if mvnum == 2 then
            ...
            if @tr.count == 0 then
              ...
              @tr.each do |tritem|
                ...
                case when mp_enrgsums[0] <= 0.9 * 167 * cosfi * (tritem.snom) then 
                       tau = 77
                       tm = 167 
                     when mp_enrgsums[0] <= 0.9 * 333 * cosfi * (tritem.snom) then 
                       tau = 200
                       tm = 333
                     when mp_enrgsums[0] <= 0.9 * 500 * cosfi * (tritem.snom) then
                       tau = 383
                       tm = 500
                     when mp_enrgsums[0] <= 0.9 * 667 * cosfi * (tritem.snom) then 
                       tau = 623
                       tm = 667
                     when mp_enrgsums[0] <= 0.9 * 730 * cosfi * (tritem.snom) then 
                       tau = 730
                       tm = 730  
                     else tau = 730
                          tm = 730    
              ...
              @report << [nil,'Потери тр-ра суммарные',nil,nil,nil,nil,nil,nil,nil,(tr_losses_kz+tr_losses_xx).round(4),2]              
            end  # if tr.count
            # линии              
